...
public class StoreValidationErrorForDts extends AbstractServiceDelegate
{
			...
			throws BpmnError, Exception
	{
		...
		String downloadUrl = saveBinary(encrypted);
		variables.setString(BPMN_EXECUTION_VARIABLE_BINARY_URL, downloadUrl);
	}

	protected String saveBinary(byte[] encryptedContent)
	{
	...
	private IdType createBinaryResource(Binary binary)
	{
		...
		catch (Exception e)
		{
			...
			throw new BpmnError(
					...
					"Unable to create Binary resource with encrypted validation error for DTS in local DSF FHIR server");
		}
	}
}
