...
public final class DataStoreFhirClientStub implements DataStoreFhirClient
{
	private static final Logger logger = LoggerFactory.getLogger(DataStoreFhirClientStub.class);
	...
	public DataStoreFhirClientStub(DataStoreClient dataStoreClient, DataLogger dataLogger)
	{
	...
	{
		logger.warn("Returning four simulated patient references, connection to Data Store FHIR server not configured");
	...
	{
		logger.warn(
				"Returning simulated Data Store FHIR resources (Patient, Condition, Observation) for {}, connection to Data Store FHIR server not configured",
				pseudonym);

		Patient p = dataStoreClient.getFhirContext().newJsonParser().parseResource(Patient.class, patient);
		...
		Condition c = dataStoreClient.getFhirContext().newJsonParser().parseResource(Condition.class, condition);
		c.setIdElement(new IdType(UUID.randomUUID().toString()));
		c.setSubject(new Reference(p.getIdElement()));

		Observation o = dataStoreClient.getFhirContext().newJsonParser().parseResource(Observation.class, observation);
	...
	{
		logger.warn("Returning simulated patient resource for {}, connection to Data Store FHIR server not configured",
				reference);
	...
	public void updatePatient(Patient patient)
	{
		...
		logger.info(
				"Not updating patient resource in Data Store FHIR server, connection to Data Store FHIR server not configured");
	}
}
